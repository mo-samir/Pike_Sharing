...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        final_inputs = np.dot(hidden_outputs,self.weights_hidden_to_output) # signals into final output layer (1*2)(2*1) >> 1
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        #hidden_error = np.dot(self.weights_hidden_to_output,output_error_term) #(2*1)(1) >> (2*1) #1#
        hidden_error = np.dot(error, self.weights_hidden_to_output.T)
        
        # TODO: Backpropagated error terms - Replace these values with your calculations.
        #output_error_term = error * self.final_outputs * (1-self.final_outputs)
        
        hidden_error_term = hidden_error * (hidden_outputs * (1-hidden_outputs)).T #(2*1) ((1*2)(1*2))T >>  (2*1)
        
        # Weight step (input to hidden)
        delta_weights_i_h += hidden_error_term * X[:, None]
        # Weight step (hidden to output)
        delta_weights_h_o += output_error_term * hidden_outputs[:, None]#hidden_outputs * output_error_term #* hidden_outputs #2#
